...
int main(int argc, char *argv[]) {
    ...
    argstream >> bottom_layers;
    argstream >> top_layers;
    argstream >> top_Au_layers;

    bottom_layers -= 1;

    int *repz = new int[3];
    int *repz_Au = new int[3];  
    int **mill = new int *[3];
    ...
    latticeconstants_1[0] = 5.4281;
    latticeconstants_1[1] = 5.4281;
    latticeconstants_1[2] = 5.4281;
    latticetype_1 = "dia";

    latticeconstants_2[0] = 4.0495;
    latticeconstants_2[1] = 4.0495;
    latticeconstants_2[2] = 4.0495;
    latticetype_2 = "fcc";
    ...
    simulation_box[0] = cell[0];
    simulation_box[1] = cell[3];
    simulation_box[2] = cell[1];
    simulation_box[3] = cell[4];
    ...
    for (int i = 0; i < n_bottom; i++) {
        ...
        for (int j = 0; j < n_bottom; j++) {
            ...
            if (dz < spacing && dz > 0.0) {
                old_spacing = spacing;
                spacing = dz;
            }
            if (spacing - old_spacing == spacing_epsilon) {
                escape = true;
            }
        }
    }
    ...
    for (int i = 0; i < n_bottom; i++) {
        ...
        for (int j = 0; j < n_bottom; j++) {
            ...
            if (dr < minimum && dr > 0.0) {
                old_minimum = minimum;
                minimum = dr;
            }
            if (minimum - old_minimum == minimum_epsilon) {
                escape = true;
            }
        }
    }
    ...
    */
    ...
    for (int i = 0; i < n_bottom; i++) {
        if (bottom_unfiltered[i][2] < (1 + spacing_epsilon) * bottom_layers * spacing) {
            ...
            for (int j = 0; j < 3; j++) {
                bottom_filtered[bot_counter][j] = bottom_unfiltered[i][j];
            }
            bot_counter++;
        }
    }
    ...
    for (int i = 0; i < n_top; i++) {
        if (top_unfiltered[i][2] < (1 + spacing_epsilon) * top_layers * spacing) {
            ...
            for (int j = 0; j < 3; j++) {
                top_filtered[top_counter][j] = top_unfiltered[i][j];
            }
            top_counter++;
        }
    }
    ...
    for (int i = 0; i < n_top_Au; i++) {
        if (top_Au_unfiltered[i][2] < (1 + spacing_epsilon) * top_Au_layers * spacing) {
            ...
            for (int j = 0; j < 3; j++) {
                top_Au_filtered[top_Au_counter][j] = top_Au_unfiltered[i][j];
            }
            top_Au_counter++;
        }
    }
    ...
    for (int i = 0; i < all_counter; i++) {
        if (all_filtered[i][2] < min) {
            min = all_filtered[i][2];
        }
        if (all_filtered[i][2] > max) {
            max = all_filtered[i][2];
        }
    }
